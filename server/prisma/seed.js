...
function parseQuantityAndUnit(inputString) {
    inputString = inputString.slice(3);
    const quantity = parseFloat(inputString.replace(/[^0-9]/g, ""));
    const unit = inputString.replace(/[0-9.]/g, "").trim();

    return { quantity, unit };
}

function processNutrients(entry) {
    const nutrients = {
...
async function main() {
    ...
    for (const entry of dataset) {
        ...
        const nutrients = processNutrients(entry);
        const { quantity, unit } = parseQuantityAndUnit(entry["Hoeveelheid/Quantity"]);
        promises.push(
